...
bool InitResourceDX10(void)
{
	...
	// vertex buffer
    cbDesc.ByteWidth = sizeof(g_Quads);
	...
	D3D10_TEXTURE_ADDRESS_MODE modes[] = 
	{
	...
	for ( int i=0; i<4; i++ )
	{
		sampler_desc.AddressU = modes[i];
		sampler_desc.AddressV = modes[i];
		sampler_desc.AddressW = modes[i];
...
void RenderFrameDX10(void)
{
	...
	*pConstData = world_view_proj_matrix;
	...
	for ( int i=0; i<4; i++ )
	{
		g_pDevice->PSSetSamplers(0, 1, &g_pSamplerState[i]);
		g_pDevice->Draw(4, i*4);
	}
	// 等待硬體掃結束, 然後才更新畫面
