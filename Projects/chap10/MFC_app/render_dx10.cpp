...
void ResizeWindowDX10(int w, int h)
{
	float aspect = (float) h / (float) w;
	Matrix4x4 view_matrix = GutMatrixLookAtRH(g_eye, g_lookat, g_up);
	Matrix4x4 projection_matrix = GutMatrixPerspectiveRH_DirectX(90.0f, aspect, 1.0f, 100.0f);
	g_view_proj_matrix = view_matrix * projection_matrix;
}
...
void RenderFrameDX10(void)
{
	...
	world_view_proj_matrix.RotateZ(angle); // 乘上旋轉矩陣
