...
bool InitResourceDX10(void)
{
	...
	// face culling object
	{
		...
		g_pDevice->CreateRasterizerState(&desc, &g_pCull);
	}
	// disable rgba output
	{
	...
	// material overwrite
	{
		...
		g_material_spotlightpass.m_bCullFace = false;
...
void RenderFrameDX10(void)
{
	...
	// stencil only pass, 在stencil buffer裏標出光源照射到的像素
	{
	...
	// 畫出被光線照射到的區域
	{
		...
		g_pDevice->RSSetState(g_pCull); // `只畫背對鏡頭三角形`
